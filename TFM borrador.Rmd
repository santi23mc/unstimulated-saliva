...
```{r}
frecu10 <- frecu[frecu$frecuencia<10,]
...
```{r}
library(corrplot)
cor_matrix <- datos.final %>% select(where(is.numeric)) %>% cor() %>% round(2)
cor_matrix
...
vardep<- "Saliva_ml_min"
nombres1<- c("Sexo", "Nº enfermedades sistemicas", "Nº de fcos", 
             "2: Neoplasms", "3: Diseases of the blood or blood-forming organs . Diseases of inmune system", 
             "05: Endocrine, nutricional or metabolic diseases", "5A00.0Z Congenital Hypothyroidism unspecified. 135", 
             ...
             "09 Diseases of the visual system", "9B10.0Z Age-related cataract, unspecified 57", 
             "10 Diseases of the ear or mastoid process", "11 Diseases of circulatory system", 
             "11BA00Z Essential Hypertension, unspecified", "11BC81 Supraventricular tachyarrhythmia   35", 
             "11BC65Z Cardiac arrhythmia associated with genetic disorder, unspecified  43", 
             ...
             "13 Diseases of digestive system", "13DA01.00 Leukoplakia", "13.EA91.40 Non-erosive oral lichen planus", 
             "14  Diseases of the skin", "15 Diseases of musculoskeletal system or connective tissue", 
             "15FA25.0 Primary Gout 24", "15 FA0Z Osteoarthritis, unspecified . Arthrosis. 10", 
             "15FB83.11 Postmenopausal osteoporosis 8", "16      Diseases of the genitourinary system", 
             "16GA91.Z Inflammatory and other diseases of prostate, unspecified  9 ( hiperplasia benigna de próstata.", 
             "21 Symptoms, signs or clinical findings not elsewhere classified", 
             "A: SISTEMA DIGESTIVO Y METABOLISMO", "A02", "A02B", "A02B C", 
             "A02BC 01", "A02BC02", "A10", "A10B", "A10BA", "A10BA02", "A10B D", 
             "A10BD 07", "A11", "A11C", "A11C C", "A11CC05", "A11CC 06", "A12", 
             "A12 A", "A12AX P1", "B: SANGRE Y ÓRGANOS HEMATOPOYÉTICOS", 
             "B01", "B01A", "B01AA 07", "B01A C", "B01AC 06", "B03", "B03 B", 
             "B03B A", "B03BA01", "C: SISTEMA CARDIOVASCULAR", "C01", "C02", 
             "C02C", "C02C A", "C02CA04", "C03", "C03 C", "C03C A", "C03 E", 
             "C03E B", "C03EB01", "C07", "C07 A", "C07A B", "C07AB 03", "C07AB07", 
             "C07B", "C08", "C08C", "C08C A", "C08CA01", "C09", "C09A", "C09A A", 
             "C09AA02", "C09AA 03", "C09B", "C09B A", "C09BA02", "C09BA 03", 
             "C09 C", "C09CA", "C09CA 01", "C09D", "C09D A", "C09DA 08", "C09D B", 
             "C09DB 01", "C09DB 02", "C09DB06", "C09D X", "C09DX 03", "C10", 
             "C10 A", "C10A A", "C10AA 01", "C10AA 05", "C10AA 07", "C10 B", 
             "C10B A", "D:MEDICAMENTOS DERMATOLÓGICOS", "G: APARATO GENITOURINARIO Y HORMONAS SEXUALES", 
             "G04", "G04C", "G04CA", "G04CA 02", "H: PREPARADOS HORMONALES SISTÉMICOS, EXCL. HORMONAS SEXUALES", 
             "H03AA 01", "M: SISTEMA MUSCULOESQUELÉTICO", "M01", "M01A", 
             "M01AX", "M04", "M04A", "M04AA", "M04AA 01", "N:SISTEMA NERVIOSO", 
             "N02", "N02 A", "N02A J", "N02 B", "N02B B", "N02BB 02", "N02B E", 
             "N02BE 01", "N03", "N03A", "N03A X", "N05", "N05B", "N05BA", 
             "N05BA 06", "N05BA08", "N05C", "N06", "N06A", "N06AB", "N06AB 06", 
             "N06AX", "R: SISTEMA RESPIRATORIO", "R03", "R03A", "R06", "R06A", 
             "R06AX", "S: ÓRGANOS DE LOS SENTIDOS", "S01", "S01E", "S01ED", 
             "Edad")
...
selec1<-stepAIC(null,scope=list(upper=full),
                direction="both",family = gaussian(link = "identity"),trace=FALSE, k=5.67)
...
rfgrid<-expand.grid(mtry=c(1,5,10,25,50,75,90,100,110,125,150))
...
rf<- train(Saliva_ml_min~ `Nº de fcos` + `11BA00Z Essential Hypertension, unspecified` + `6B0Z Anxiety or fear- related disorders, unspecified 3` + A10 + A10BA + Edad,data=datos_train,
           method="rf",trControl=control,tuneGrid=rfgrid,
           linout = T,ntree=500,nodesize=10,replace=TRUE,
           importance=TRUE)
...
gbmgrid<-expand.grid(shrinkage=c(0.1),
                     n.minobsinnode=c(10),
                     n.trees=c(4,5,25,50,75,100,150,300,600,100,1500),
...
gbm<- train(Saliva_ml_min~`Nº de fcos` + `11BA00Z Essential Hypertension, unspecified` + `6B0Z Anxiety or fear- related disorders, unspecified 3` + A10 + A10BA + Edad,data=datos_train,
            method="gbm",trControl=control,tuneGrid=gbmgrid,
            bag.fraction=1,verbose=FALSE)
...
modelosbf <- train(Saliva_ml_min ~ `Nº de fcos` + `5A10/ 5A11 Diabetes Mellitus 133` +
...
`13DA01.00 Leukoplakia` + `13.EA91.40 Non-erosive oral lichen planus` + 
`14  Diseases of the skin` + `15 Diseases of musculoskeletal system or connective tissue` + 
`15 FA0Z Osteoarthritis, unspecified . Arthrosis. 10` + `A: SISTEMA DIGESTIVO Y METABOLISMO` + 
 A10 + A10B + `A10BD 07` + A12 + `A12 A` + `B01AA 07` + `C: SISTEMA CARDIOVASCULAR` + 
 C07 + `C07 A` + C09 + `N:SISTEMA NERVIOSO` + N05B + R03A, data = datos_train, method = "lm")


summary(modelosbf)
summary(modelosbf)$r.squared
...
modeloaic <- train(Saliva_ml_min ~ Edad + `14  Diseases of the skin` + A10 + 
`15 Diseases of musculoskeletal system or connective tissue` + 
`6B0Z Anxiety or fear- related disorders, unspecified 3` + `13DA01.00 Leukoplakia` + R03A + C09B + R06 + `10 Diseases of the ear or mastoid process` + `C10 A` + `13.EA91.40 Non-erosive oral lichen planus` + N06AX + N06A + `B03 B` + C08C + `G04CA 02` + C07 + `A10BD 07` + `A10B D` + A10BA + G04 + Sexo, data = datos_train, method = "lm")
...
modelobic <- train(Saliva_ml_min ~ `14  Diseases of the skin` + A10 + 
Edad + `15 Diseases of musculoskeletal system or connective tissue` + `6B0Z Anxiety or fear- related disorders, unspecified 3`, data = datos_train, method = "lm")


summary(modelobic)
summary(modelobic)$r.squared
...
modeloboruta <- train( Saliva_ml_min ~ `Nº de fcos` + `11BA00Z Essential Hypertension, unspecified` + `6B0Z Anxiety or fear- related disorders, unspecified 3` + A10 + A10BA + Edad, data = datos_train, method = "lm")


summary(modeloboruta)
summary(modeloboruta)$r.squared
...
SVM<- train(Saliva_ml_min~`Nº de fcos` + `5A10/ 5A11 Diabetes Mellitus 133` +
`06: Mental, behavioural or neurodevelopmental disorders` + 
`6B0Z Anxiety or fear- related disorders, unspecified 3` + `11 Diseases of circulatory system` + 
`11BA00Z Essential Hypertension, unspecified` + `13 Diseases of digestive system` + 
`13DA01.00 Leukoplakia` + `13.EA91.40 Non-erosive oral lichen planus` + 
...
`15 FA0Z Osteoarthritis, unspecified . Arthrosis. 10` + `A: SISTEMA DIGESTIVO Y METABOLISMO` + 
 A10 + A10B + `A10BD 07` + A12 + `A12 A` + `B01AA 07` + `C: SISTEMA CARDIOVASCULAR` + 
 C07 + `C07 A` + C09 + `N:SISTEMA NERVIOSO` + N05B + R03A,data=datos_train,
 method="svmLinear",trControl=control,
 tuneGrid=SVMgrid,verbose=FALSE)
...
SVM<- train(Saliva_ml_min~ Edad + `14  Diseases of the skin` + A10 + 
`15 Diseases of musculoskeletal system or connective tissue` + 
`6B0Z Anxiety or fear- related disorders, unspecified 3` + `13DA01.00 Leukoplakia` + R03A + C09B + R06 + `10 Diseases of the ear or mastoid process` + `C10 A` + `13.EA91.40 Non-erosive oral lichen planus` + N06AX + N06A + `B03 B` + C08C + `G04CA 02` + C07 + `A10BD 07` + `A10B D` + A10BA + G04 + Sexo,data=datos_train,
 method="svmLinear",trControl=control,
 tuneGrid=SVMgrid,verbose=FALSE)
...
SVM<- train(Saliva_ml_min~ `14  Diseases of the skin` + A10 + 
Edad + `15 Diseases of musculoskeletal system or connective tissue` + `6B0Z Anxiety or fear- related disorders, unspecified 3`,data=datos_train,
 method="svmLinear",trControl=control,
 tuneGrid=SVMgrid,verbose=FALSE)
...
SVM<- train(Saliva_ml_min~`Nº de fcos` + `11BA00Z Essential Hypertension, unspecified` + `6B0Z Anxiety or fear- related disorders, unspecified 3` + A10 + A10BA + Edad ,data=datos_train,
 method="svmLinear",trControl=control,
 tuneGrid=SVMgrid,verbose=FALSE)
...
SVMpoly<- train(Saliva_ml_min~`Nº de fcos` + `5A10/ 5A11 Diabetes Mellitus 133` +
`06: Mental, behavioural or neurodevelopmental disorders` + 
`6B0Z Anxiety or fear- related disorders, unspecified 3` + `11 Diseases of circulatory system` + 
`11BA00Z Essential Hypertension, unspecified` + `13 Diseases of digestive system` + 
`13DA01.00 Leukoplakia` + `13.EA91.40 Non-erosive oral lichen planus` + 
...
SVMpoly<- train(Saliva_ml_min~ Edad + `14  Diseases of the skin` + A10 + 
`15 Diseases of musculoskeletal system or connective tissue` + 
`6B0Z Anxiety or fear- related disorders, unspecified 3` + `13DA01.00 Leukoplakia` + R03A + C09B + R06 + `10 Diseases of the ear or mastoid process` + `C10 A` + `13.EA91.40 Non-erosive oral lichen planus` + N06AX + N06A + `B03 B` + C08C + `G04CA 02` + C07 + `A10BD 07` + `A10B D` + A10BA + G04 + Sexo,data=datos_train,
   method="svmPoly",trControl=control,
   tuneGrid=SVMpolygrid,verbose=FALSE)
...
SVMpoly<- train(Saliva_ml_min~`14  Diseases of the skin` + A10 + 
Edad + `15 Diseases of musculoskeletal system or connective tissue` + `6B0Z Anxiety or fear- related disorders, unspecified 3` ,data=datos_train,
   method="svmPoly",trControl=control,
   tuneGrid=SVMpolygrid,verbose=FALSE)
...
SVMpoly<- train(Saliva_ml_min~ `Nº de fcos` + `11BA00Z Essential Hypertension, unspecified` + `6B0Z Anxiety or fear- related disorders, unspecified 3` + A10 + A10BA + Edad,data=datos_train,
   method="svmPoly",trControl=control,
   tuneGrid=SVMpolygrid,verbose=FALSE)
...
SVMrbf<- train(Saliva_ml_min~`Nº de fcos` + `5A10/ 5A11 Diabetes Mellitus 133` +
`06: Mental, behavioural or neurodevelopmental disorders` + 
`6B0Z Anxiety or fear- related disorders, unspecified 3` + `11 Diseases of circulatory system` + 
`11BA00Z Essential Hypertension, unspecified` + `13 Diseases of digestive system` + 
`13DA01.00 Leukoplakia` + `13.EA91.40 Non-erosive oral lichen planus` + 
...
SVMrbf<- train(Saliva_ml_min~ Edad + `14  Diseases of the skin` + A10 + 
`15 Diseases of musculoskeletal system or connective tissue` + 
`6B0Z Anxiety or fear- related disorders, unspecified 3` + `13DA01.00 Leukoplakia` + R03A + C09B + R06 + `10 Diseases of the ear or mastoid process` + `C10 A` + `13.EA91.40 Non-erosive oral lichen planus` + N06AX + N06A + `B03 B` + C08C + `G04CA 02` + C07 + `A10BD 07` + `A10B D` + A10BA + G04 + Sexo,data=datos_train,
   method="svmRadial",trControl=control,
   tuneGrid=SVMrbfgrid,verbose=FALSE)
...
SVMrbf<- train(Saliva_ml_min~`14  Diseases of the skin` + A10 + 
Edad + `15 Diseases of musculoskeletal system or connective tissue` + `6B0Z Anxiety or fear- related disorders, unspecified 3`,data=datos_train,
   method="svmRadial",trControl=control,
   tuneGrid=SVMrbfgrid,verbose=FALSE)
...
SVMrbf<- train(Saliva_ml_min~`Nº de fcos` + `11BA00Z Essential Hypertension, unspecified` + `6B0Z Anxiety or fear- related disorders, unspecified 3` + A10 + A10BA + Edad,data=datos_train,
   method="svmRadial",trControl=control,
   tuneGrid=SVMrbfgrid,verbose=FALSE)
...
medias1<-cruzadalin(data=data,
                    vardep="Saliva_ml_min",listconti=
                      c("Nº de fcos", "5A10/ 5A11 Diabetes Mellitus 133", 
                        "06: Mental, behavioural or neurodevelopmental disorders", 
                        "6B0Z Anxiety or fear- related disorders, unspecified 3", 
                        "11 Diseases of circulatory system", 
                        "11BA00Z Essential Hypertension, unspecified", 
                        "13 Diseases of digestive system", "13DA01.00 Leukoplakia", 
                        "13.EA91.40 Non-erosive oral lichen planus", "14  Diseases of the skin", 
                        "15 Diseases of musculoskeletal system or connective tissue", 
                        "15 FA0Z Osteoarthritis, unspecified . Arthrosis. 10", 
                        "A: SISTEMA DIGESTIVO Y METABOLISMO", "A10", "A10B", "A10BD 07", "A12", "A12 A",                          "B01AA 07", "C: SISTEMA CARDIOVASCULAR", "C07", "C07 A", "C09", 
                        "N:SISTEMA NERVIOSO", "N05B", "R03A"),
...
medias2<-cruzadalin(data=data,
                    vardep="Saliva_ml_min",listconti=
                      c("Edad", "14  Diseases of the skin", "A10", 
                        "15 Diseases of musculoskeletal system or connective tissue", 
                        "6B0Z Anxiety or fear- related disorders, unspecified 3", 
                        "13DA01.00 Leukoplakia", "R03A", "C09B", "R06", 
                        "10 Diseases of the ear or mastoid process", "C10 A", 
                        "13.EA91.40 Non-erosive oral lichen planus", "N06AX", "N06A", "B03 B",                                  "C08C", "G04CA 02", "C07", "A10BD 07", "A10B D", "A10BA", "G04", "Sexo"),
                    listclass=c(""),grupos=4,sinicio=1234,repe=25)
...
medias4<-cruzadalin(data=data,
                    vardep="Saliva_ml_min",listconti=
                      c("Nº de fcos", "6B0Z Anxiety or fear- related disorders, unspecified 3", 
                        "11BA00Z Essential Hypertension, unspecified", "A10", "A10BA", "Edad"),
                    listclass=c(""),grupos=4,sinicio=1234,repe=25)
...
medias5<-cruzadarf(data=data,
                    vardep="Saliva_ml_min",listconti=
                      c("Nº de fcos", "6B0Z Anxiety or fear- related disorders, unspecified 3", 
                        "11BA00Z Essential Hypertension, unspecified", "A10", "A10BA", "Edad"),
                    listclass=c(""),grupos=4,sinicio=1234,repe=25,mtry=1,ntree=500,nodesize=10)
...
medias10 <- cruzadaSVM(data=data,
                    vardep="Saliva_ml_min",listconti=
                      c("Nº de fcos", "6B0Z Anxiety or fear- related disorders, unspecified 3", 
                        "11BA00Z Essential Hypertension, unspecified", "A10", "A10BA", "Edad"),
                    listclass=c(""),grupos=4,sinicio=1234,repe=25, C=25)
...
medias11 <- cruzadaSVMpoly(data=data,
                    vardep="Saliva_ml_min",listconti=
                      c("Nº de fcos", "5A10/ 5A11 Diabetes Mellitus 133", 
                        ...
                        "A: SISTEMA DIGESTIVO Y METABOLISMO", "A10", "A10B", "A10BD 07", "A12", "A12 A",                          "B01AA 07", "C: SISTEMA CARDIOVASCULAR", "C07", "C07 A", "C09", 
                        "N:SISTEMA NERVIOSO", "N05B", "R03A"),
                    listclass=c(""),grupos=4,sinicio=1234,repe=25,C=0.0016,scale=2,degree=1)
...
medias12 <- cruzadaSVMpoly(data=data,
                    vardep="Saliva_ml_min",listconti=
                      c("Edad", "14  Diseases of the skin", "A10", 
                        ...
                        "10 Diseases of the ear or mastoid process", "C10 A", 
                        "13.EA91.40 Non-erosive oral lichen planus", "N06AX", "N06A", "B03 B",                                  "C08C", "G04CA 02", "C07", "A10BD 07", "A10B D", "A10BA", "G04", "Sexo"),
                    listclass=c(""),grupos=4,sinicio=1234,repe=25,C=0.0016,scale=5,degree=1)
...
medias13 <- cruzadaSVMpoly(data=data,
                    vardep="Saliva_ml_min",listconti=
                      c("Edad", "14  Diseases of the skin", "A10", 
                        "15 Diseases of musculoskeletal system or connective tissue", 
                        "6B0Z Anxiety or fear- related disorders, unspecified 3"),
                    listclass=c(""),grupos=4,sinicio=1234,repe=25,C=0.2,scale=0.1,degree=1)
...
medias14 <- cruzadaSVMpoly(data=data,
                    vardep="Saliva_ml_min",listconti=
                      c("Nº de fcos", "6B0Z Anxiety or fear- related disorders, unspecified 3", 
                        "11BA00Z Essential Hypertension, unspecified", "A10", "A10BA", "Edad"),
                    listclass=c(""),grupos=4,sinicio=1234,repe=25,C=10,scale=1,degree=1)
...
medias15 <- cruzadaSVMRBF(data=data,
                    vardep="Saliva_ml_min",listconti=
                      c("Nº de fcos", "5A10/ 5A11 Diabetes Mellitus 133", 
                        ...
                        "A: SISTEMA DIGESTIVO Y METABOLISMO", "A10", "A10B", "A10BD 07", "A12", "A12 A",                          "B01AA 07", "C: SISTEMA CARDIOVASCULAR", "C07", "C07 A", "C09", 
                        "N:SISTEMA NERVIOSO", "N05B", "R03A"),
                    listclass=c(""),grupos=4,sinicio=1234,repe=25,C=0.2,sigma=0.008)

medias15$modelo="SVM Radial SBF"
...
medias16 <- cruzadaSVMRBF(data=data,
                    vardep="Saliva_ml_min",listconti=
                      c("Edad", "14  Diseases of the skin", "A10", 
                        ...
                        "10 Diseases of the ear or mastoid process", "C10 A", 
                        "13.EA91.40 Non-erosive oral lichen planus", "N06AX", "N06A", "B03 B",                                  "C08C", "G04CA 02", "C07", "A10BD 07", "A10B D", "A10BA", "G04", "Sexo"),
                    listclass=c(""),grupos=4,sinicio=1234,repe=25,C=1,sigma=0.008)

medias16$modelo="SVM Radial AIC"
...
medias17$modelo="SVM Radial BIC"
...
medias18$modelo="SVM Radial Boruta"
...
```
```{r}
union5<-rbind(medias10,medias14,medias18)
par(cex.axis=0.6)
...
```{r}
listado <- c(  "predi9", "predi10", "predi11", "predi16", 
